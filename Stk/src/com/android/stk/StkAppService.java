...
//Begin added by cnaxuan.zeng for XR7176933 on 2018/12/04
import com.tcl.plugin.SimLockUtils;
import com.tcl.plugin.TclPluginManager;
//End added by cnaxuan.zeng for XR7176933 on 2018/12/04
...
public class StkAppService extends Service implements Runnable {
    ...
    protected class StkContext {
	 ...
	 //Begin added by donghai.wu for XR6701129   on 2018/09/28
	 //Process session end when alpha information is empty.
        private boolean mEmptyAlphaEnd = false;
	 //End added by donghai.wu for  XR6701129 on   on 2018/09/28
    ...
    private final BroadcastReceiver mSpnReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
		    ...
		    CatLog.d(LOG_TAG, "mSpnReceiver  simId:" + simId+"mPLMN:"+StkApp.mPLMN[simId]);
                }
            }
        }
    };	
    ...
    //Begin added by donghai.wu  for XR6154319   on 2018/09/28
    public static class StkResetReceiver extends BroadcastReceiver{
        @Override
        public void onReceive(Context context, Intent intent) {
            final String evtAction = intent.getAction();

            CatLog.d(LOG_TAG, "++++++ mStkResetReceiver  evtAction:" + evtAction);
            if (true == ResUtils.getBoolean(context, "def_stk_install_when_simcard_has_stkmenu", false)) {
                CatLog.d(LOG_TAG, "++++++  def_stk_install_when_simcard_has_stkmenu is true");
                if (StkUtils.getInstance().isContainStkMenu()) {
                    CatLog.d(LOG_TAG, "++++++  mStkReset is true onReceive reset");
                ...
                if (StkUtils.getInstance().isSetStkCommonIcon()){
                    CatLog.d(LOG_TAG, "++++++ isSetStkCommonIcon do not refresh stk app");
                } else {
                    CatLog.d(LOG_TAG, "++++++ mStkReset is false reset");
                    StkAppInstaller.unInstall(context);
                    StkAppInstaller.install(context);
                }
    ...
    public void onCreate() {
        ...
        //Begin added by canxuan.zeng for XR7176933 on 2018/12/04
        mSIMStateChangeFilter.addAction(SimLockUtils.TCL_ACTION_SUBINFO_RECORD_UPDATED);
        //End added by canxuan.zeng for XR7176933 on 2018/12/04
...
//Begin added by zubin.chen.hz for XR7135244 on 2018/11/22
        StkUtils.getInstance().initStkService(mContext);
//End added by zubin.chen.hz for XR7135244 on 2018/11/22
    ...
    public void initNotify() {
        int i = 0;
        for (i = 0; i < mSimCount; i++) {
            StkApp.mPLMN[i] = StkUtils.getInstance().getDefaultPlmn(mContext);
        }
        return;
    }
    //End added by donghai.wu for  XR7230031 on 2018/12/19
    ...
    public void onStart(Intent intent, int startId) {
        ...
        if ((slotId >= 0 && slotId < mSimCount) && mStkService[slotId] == null) {
            ...
            if (i == mSimCount) {
                ...
                StkUtils.getInstance().unInstallStk(mContext);
    ...
    private String getTelephonyPlmnFrom(Intent intent) {
        if (intent.getBooleanExtra(TelephonyIntents.EXTRA_SHOW_PLMN, false)) {
            ...
            if (plmn != null && !plmn.equalsIgnoreCase(getResources().getString(
                   com.android.internal.R.string.lockscreen_carrier_default))) {
                return plmn;
            }
        }
        return StkUtils.getInstance().getDefaultPlmn(mContext);
    }
    //End added by donghai.wu for  XR7230031 on 2018/12/19
    /*
    ...
    boolean isHasMaincmd(int slotId)
    {
        if ((mStkContext[slotId]!= null)&&(mStkContext[slotId].mMainCmd != null)){
	     CatLog.d(LOG_TAG, "isHasMaincmd  mMainCmd != null");
            return true;
	  }
	  else{
	     CatLog.d(LOG_TAG, "isHasMaincmd  mMainCmd == null");
    ...
    private final class ServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            case OP_BOOT_COMPLETED:
                ...
                if (i == mSimCount) {
                    // Begin modified by donghai.wu  for XR6154319    on 2018/09/28
                    //StkAppInstaller.unInstall(mContext);
                    CatLog.d(LOG_TAG, " OP_BOOT_COMPLETED no stk");
			 // End modified by donghai.wu  for XR6154319    on 2018/09/28
                }
                break;
            case OP_DELAYED_MSG:
        ...
        private void handleCardStatusChangeAndIccRefresh(Bundle args, int slotId) {
	     ...
	     CatLog.d(LOG_TAG, "CardStatus: " + cardStatus);
            if (cardStatus == false) {
		  //Begin added by donghai.wu for XR7072230  on 2018/12/08
                  String NOT_REFRESH =  "notRefresh";
		  String IS_RESET = "isReset";
		  String NOT_RESET = "notReset";
		  String type = null;
		  type =  SystemProperties.get("gsm.sim_refresh_type",NOT_REFRESH);
		  CatLog.d(LOG_TAG,"Refresh type =" + type);
		  if (type.equals(NOT_RESET)){
                    SystemProperties.set("gsm.sim_refresh_type",NOT_REFRESH);
		      return;
		  }
		  //End added by donghai.wu for XR7072230  on 2018/12/08
               CatLog.d(LOG_TAG, "cardStatus == false CARD is ABSENT");
		   ...
		   //Begin added by donghai.wu for XR7072230  on 2018/12/08
                if ((isAllOtherCardsAbsent(slotId))&&(type.equals(NOT_REFRESH))){
		   //End added by donghai.wu for XR7072230  on 2018/12/08
                    CatLog.d(LOG_TAG, "cardStatus == false All CARDs are ABSENT");
	             // Begin modified by donghai.wu  for XR6154319    on 2018/09/28
                    //StkAppInstaller.unInstall(mContext);
                    StkUtils.getInstance().unInstallStk(mContext);
                    //stopSelf();
    ...
    private void handleSessionEnd(int slotId) {
        ...
        //Begin modified by donghai.wu for XR6701129   on 2018/09/28
        //Process session end when alpha information is empty.
        if (StkMenuActivity.STATE_SECONDARY == mStkContext[slotId].mMenuState
          || (mStkContext[slotId].mEmptyAlphaEnd && mStkContext[slotId].mMenuIsVisible)) {
                launchMenuActivity(null, slotId);
        }
	 //End modified by donghai.wu for XR6701129  on 2018/09/28
        if (mStkContext[slotId].mCmdsQ.size() != 0) {
            callDelayedMsg(slotId);
        } else {
            mStkContext[slotId].mCmdInProgress = false;
    ...
    private boolean isTopOfStack() {
	  ...
	  //Begin added by donghai.wu for XR6701127  on 2018/09/28
        if(mActivityManager.getRunningTasks(1) == null ||
              mActivityManager.getRunningTasks(1).isEmpty()) {
              return false;
	 }
	 //End added by donghai.wu for  XR6701127  on 2018/09/28
    ...
    private void handleCmd(CatCmdMessage cmdMsg, int slotId) {
        ...
        case SET_UP_MENU:
            ...
            if (removeMenu(slotId)) {
                ...
                //Begin modified by donghai.wu for XR7307271 on 2019/01/02
                for (i = PhoneConstants.SIM_ID_1; i < mSimCount; i++) {
                    if (i != slotId
		...
		//End modified by donghai.wu for XR7307271 on 2018/01/02
                if (i == mSimCount) {
                    // Begin modified by donghai.wu  for XR6154319    on 2018/09/28
                    //StkAppInstaller.unInstall(mContext);
                    StkUtils.getInstance().unInstallStk(mContext);
        ...
        case LAUNCH_BROWSER:
	      ...
	      //Begin modified by donghai.wu for XR7100498 on 2018/11/06
           ...
           // } else {
            //End modified by donghai.wu for XR7100498  on 2018/11/06
    ...
    private void handleCmdResponse(Bundle args, int slotId) {
        ...
        case RES_ID_INPUT:
            ...
            if (input != null && (null != mStkContext[slotId].mCurrentCmd.geInput()) &&
                 ...
                 //Begin modified by donghai.wu for XR6701075  on 2018/09/28
                if (helpRequired) {
                    resMsg.setResultCode(ResultCode.HELP_INFO_REQUIRED);
                } else {
                ...
                //End modified by donghai.wu for XR6701075  on 2018/09/28
                ...
                if (helpRequired) {
                    ...
                    resMsg.setResultCode(mStkContext[slotId].mCurrentCmd.hasIconLoadFailed() ?
                            ResultCode.PRFRMD_ICON_NOT_DISPLAYED : ResultCode.OK);
			 //Beginadded by donghai.wu  for XR6154319   on 2018/09/28
                    StkUtils.getInstance().inputSetResultCode(resMsg);
                    //End added by donghai.wu  for XR6154319   on 2018/09/28
    ...
    private void cleanUpInstanceStackBySlot(int slotId) {
	 ...
	 //Begin added by donghai.wu for XR6701129  on 2018/09/28
            ...
            mStkContext[slotId].mEmptyAlphaEnd = true;
        }
	 //End added by donghai.wu for  XR6701129  on 2018/09/28
        if (dialog != null) {
...
//Begin modified by zubin.chen.hz for XR7137257 telecomcode on 2018/12/05
        //Begin modified by donghai.wu for XR6701129  on 2018/09/28
        //Process session end when alpha information is empty.
...
//Begin modified by zubin.chen.hz for XR7137257 telecomcode on 2018/12/05
                    ...
                    //Begin modified by donghai.wu for XR6701129  on 2018/09/28
                    //Process session end when alpha information is empty.
                   /* if (!mStkContext[slotId].mEmptyAlphaEnd) {
                        return;
                    } else {
                        mStkContext[slotId].mEmptyAlphaEnd = false;
                    }*/
                    //End modified by donghai.wu for XR6701129  on 2018/09/28
//Begin modified by zubin.chen.hz for XR7137257 telecomcode on 2018/12/05
                }
    ...
    private void launchInputActivity(int slotId) {
        ...
        Uri uriData = Uri.parse(uriString);
	  //Begin added by donghai.wu  for XR6154319   on 2018/09/28
        StkUtils.getInstance().beforeLaunchInputActivity
                (mContext,mStkContext[slotId].lastSelectedItem, mStkContext[slotId].mMainCmd, mStkContext[slotId].mCurrentCmd, slotId);
        //End added by donghai.wu  for XR6154319   on 2018/09/28
    ...
    private void launchTextDialog(int slotId) {
        ...
        //Begin add by canxuan.zeng for XR7143706 on 2018/11/27
        StkUtils.getInstance().beforeLaunchDialogActivity
                (mContext, mStkContext[slotId].mCurrentCmd, slotId);
        //End add by canxuan.zeng for XR7143706 on 2018/11/27
    ...
    private void launchEventMessage(int slotId, TextMessage msg) {
       ...
       //End modified by donghai.wu for XR7307271 on 2018/01/02
        ...
        //Begin added by donghai.wu  for XR6626333  on 2018/09/28
        if (StkUtils.getInstance().hideSmsToast(mStkContext[slotId].mCurrentCmd.getCmdType().value(), 
        		msg.text)) {
        	CatLog.d(LOG_TAG, "hide the default SEND_SMS toast string");
        	return;
        }
        //End added by donghai.wu  for XR6626333   on 2018/09/28
    ...
    private void launchIdleText(int slotId) {
        ...
        //Begin modified by donghai.wu for XR7072236 on 2018/10/30
        if (msg == null || msg.text ==null) {
            ...
            //Begin modified by donghai.wu for XR6440096 on 2018/06/20
            RemoteViews contentView; 
            if (true == ResUtils.getBoolean(mContext, "def_stk_SetUpIdleModeText_notification_color_change", false)) {
               contentView = new RemoteViews(
                        PACKAGE_NAME,
                        R.layout.status_bar_latest_event_content_stk_customize);
            } else {
               contentView = new RemoteViews(
                        PACKAGE_NAME,
                        com.android.internal.R.layout.status_bar_latest_event_content);
            }
            //End modified by donghai.wu for XR6440096  on 2018/06/20
            ...
            Intent notificationIntent = new Intent(mContext, StkNotificationAlertActivity.class);
	    ...
	    notificationBuilder.setContent(contentView);
            //Begin modified by donghai.wu for XR7099422 on 2018/11/05
            PendingIntent pendingIntent = PendingIntent.getActivity(mContext, 0,
                    notificationIntent, PendingIntent.FLAG_CANCEL_CURRENT);
	     //End modified by donghai.wu for XR7099422 on 2018/11/05
            notificationBuilder.setContentIntent(pendingIntent);
            mNotificationManager.notify(getNotificationId(slotId), notificationBuilder.build());
        }
	  //End modified by donghai.wu for XR7072236 on 2018/10/30
    ...
    private void handlePlayTone(int slotId) {
	  ...
	  // Begin added by donghai.wu  for XR6154319   on 2018/09/28
        showUser=StkUtils.getInstance().isShowUser(showUser,mStkContext[slotId].mCurrentCmd);
        // Endadded by donghai.wu  for XR6154319   on 2018/09/28
        ...
        public void onReceive(Context context, Intent intent) {
             if (TelephonyIntents.ACTION_SIM_STATE_CHANGED.equals(intent.getAction())) {
                 ...
                 if(IccCardConstants.INTENT_VALUE_ICC_LOADED.equals(simState)) {
                     StkUtils.getInstance().setStkCommonIcon(context);
                 }
             }
             //Begin added by canxuan.zeng for XR7176933 on 2018/12/04
              else if(intent.getAction().equals(SimLockUtils.TCL_ACTION_SUBINFO_RECORD_UPDATED)) {
                 CatLog.d(LOG_TAG, "++++++ get ACTION_SUBINFO_RECORD_UPDATED mSimCount: " + mSimCount);
                 int detectType = intent.getIntExtra(SimLockUtils.INTENT_KEY_DETECT_STATUS,  SimLockUtils.EXTRA_VALUE_NOCHANGE);
                 if (detectType == SimLockUtils.EXTRA_VALUE_REMOVE_SIM){
                     CatLog.d(LOG_TAG, "++++++  SIM removed, reset the properties");
                     TclPluginManager.getTclStk(mContext).handleSIMRemove();
    ...
    public void  returnHomeScreen(Context context) {
        Intent intent = new Intent(Intent.ACTION_MAIN);
        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        intent.addCategory(Intent.CATEGORY_HOME);
        startActivity(intent);
   }
   // End added by donghai.wu  for XR6625818   on 2018/09/28
}
