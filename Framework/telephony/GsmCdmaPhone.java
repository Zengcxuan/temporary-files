...
//Begin modified by lichun.xiu for XR6623928 tclPlugin on 2018/10/06
import com.tcl.plugin.TclPluginManager;
import android.util.ResUtils;
//end modified by lichun.xiu for XR6623928 tclPlugin on 2018/10/06

//Begin added by yangning.hong.hz for XR7076494 telecomcode on 2018/10/31
import com.tcl.plugin.SimLockUtils;
//End added by yangning.hong.hz for XR7076494 telecomcode on 2018/10/31
...
public class GsmCdmaPhone extends Phone {
    ...
    // Begin added by guoqi.zou for XR7098440&7098464 telecomcode on 2018/11/16
    private AsyncResult mCachedSsn = null;
    // End added by guoqi.zou for XR7098440&7098464 telecomcode on 2018/11/16
    ...
    public void registerForSuppServiceNotification(
        ...
        // End added by guoqi.zou for XR7098440&7098464 telecomcode on 2018/11/16
        // Begin deleted by guoqi.zou for XR7098440&7098464 telecomcode on 2018/11/16
//        if (mSsnRegistrants.size() == 1) mCi.setSuppServiceNotifications(true, null);
        // End deleted by guoqi.zou for XR7098440&7098464 telecomcode on 2018/11/16
    }

    @Override
    public void unregisterForSuppServiceNotification(Handler h) {
        mSsnRegistrants.remove(h);
        mCachedSsn = null; // Added by guoqi.zou for XR7098440&7098464 telecomcode on 2018/11/16
        // Begin deleted by guoqi.zou for XR7098440&7098464 telecomcode on 2018/11/16
   ...
   @Override
    ...
    private void storeVoiceMailNumber(String number) {
        ...
        if (isPhoneTypeGsm()) {
            ...
            TclPluginManager.getTclVoicemail(mContext).storeVoiceMailNumber(mContext,number, getPhoneId(), getSubscriberId());
            //End modified by lichun.xiu for XR6623928 tclPlugin on 2018/10/06
        } else {
    ...
    public String getVoiceMailNumber() {
        ...
        if (isPhoneTypeGsm()) {
            ...
            //Begin modified by lichun.xiu for XR6623928 tclPlugin on 2018/10/06
            number = TclPluginManager.getTclVoicemail(mContext).getVoiceMailNumber(mContext,getPhoneId(), getSubscriberId(), mIccRecords);
            //End modified by lichun.xiu for XR6623928 tclPlugin on 2018/10/06
        } else {
            SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(getContext());
    ...
    private void setVmSimImsi(String imsi) {
        ...
        //Begin added by lichun.xiu for XR6623928 tclPlugin on 2018/10/06
        /*
        refresh IMSI when loading sim records will cause voice mail sequence error in EVENT_SIM_RECORDS_LOADED.
        */
        if(!getIccRecords().getRecordsLoaded()) {
            Rlog.d(LOG_TAG, "SIM record is loading , ignore set VM imsi.");
            return;
        }
        //End added by lichun.xiu for XR6623928 tclPlugin on 2018/10/06
    ...
    public String getVoiceMailAlphaTag() {
        ...
        if (ret == null || ret.length() == 0) {
            //Begin modified by lichun.xiu for XR6623928 tclPlugin on 2018/10/06
            String voiceMailName = ResUtils.getString(mContext,"def_sms_voice_mail_name","");
            if (TextUtils.isEmpty(voiceMailName)) {
                return mContext.getText(com.android.internal.R.string.defaultVoiceMailAlphaTag)
                        .toString();
            } else {
    ...
    public String getDeviceSvn() {
        if (isPhoneTypeGsm() || isPhoneTypeCdmaLte()) {
            //Begin modified by yangning.hong.hz for XR7143736 telecomcode on 2018/12/12
            //return mImeiSv;
            TelephonyManager mTm = TelephonyManager.from(mContext);
            String imeiSv = mTm.getTelephonyProperty(getPhoneId(), "gsm.device.imeisv", "");
            return imeiSv;
            //End modified by yangning.hong.hz for XR7143736 telecomcode on 2018/12/12
        } else {
    ...
    public String getImei() {
        ...
        return mImei;*/
        TelephonyManager mTm = TelephonyManager.from(mContext);
        String imei = mTm.getTelephonyProperty(getPhoneId(), "gsm.device.imei", "");
        if (enable14DigitImei && !TextUtils.isEmpty(imei)
                && imei.length() > IMEI_14_DIGIT) {
            return imei.substring(0, IMEI_14_DIGIT);
        }
        return imei;
        //End modified by yangning.hong.hz for XR7143736 telecomcode on 2018/12/12
    }
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case EVENT_GET_DEVICE_IDENTITY_DONE:{
                ...
                mEsn  =  respId[2];
                mMeid =  respId[3];
                //Begin added by yangning.hong.hz for XR7143736 telecomcode on 2018/12/12
                TelephonyManager mTm = TelephonyManager.from(mContext);
                String stackid_0 = SystemProperties.get("persist.vendor.radio.msim.stackid_0");
                String stackid_1 = SystemProperties.get("persist.vendor.radio.msim.stackid_1");
                logd("EVENT_GET_DEVICE_IDENTITY_DONE stackid_0="+stackid_0+", stackid_1="+stackid_1+", phoneId="+getPhoneId());
                if ("0".equals(stackid_1)) {
                    if (getPhoneId() == PhoneConstants.SUB1) {
                        mTm.setTelephonyProperty(PhoneConstants.SUB2, "gsm.device.imei", mImei);
                        mTm.setTelephonyProperty(PhoneConstants.SUB2, "gsm.device.imeisv", mImeiSv);
                    } else {
                        mTm.setTelephonyProperty(PhoneConstants.SUB1, "gsm.device.imei", mImei);
                        mTm.setTelephonyProperty(PhoneConstants.SUB1, "gsm.device.imeisv", mImeiSv);
                    }
                } else {
                    if (getPhoneId() == PhoneConstants.SUB1) {
                        mTm.setTelephonyProperty(PhoneConstants.SUB1, "gsm.device.imei", mImei);
                ...
                //End added by yangning.hong.hz for XR7143736 telecomcode on 2018/12/12
            }
            break;
            ...
            case EVENT_CARRIER_CONFIG_CHANGED:
                ...
                if (getIccRecordsLoaded()) {
                    ...
                    //Begin added by cong.tao.hz for XR7305555 telecomcode on 2019/01/09
                    boolean mDisableUt = SystemProperties.getBoolean("ro_ss_csfb_no_ims", false);
                    if (mDisableUt) {
                        logd("SS CSFB, disable ut");
                        ImsManager.getInstance(mContext, mPhoneId).setUtEnabled(false);
                    }
                    //End added by cong.tao.hz for XR7305555 telecomcode on 2019/01/09
                } else {
            ...
            case EVENT_SIM_RECORDS_LOADED:
                ...
                //Begin modified by lichun.xiu for XR6623928 tclPlugin on 2018/10/06
                //Begin modified by lichun.xiu for XR7442071 on 2019/02/14
                TclPluginManager.getTclVoicemail(mContext).setVoicemailSimLoaded(mContext,getPhoneId(),getSubscriberId(),mIccRecords.get().getServiceProviderName());
                //End modified by lichun.xiu for XR7442071 on 2019/02/14
                ...
                //End modified by lichun.xiu for XR6623928 tclPlugin on 2018/10/06
                //Begin added by cong.tao.hz for XR7222334 telecomcode on 2018/12/19
                String userAgent = SystemProperties.get("def_sip_user_agent");
                if (DBG) logd("Sip User-Agent: " + userAgent);
                if (!TextUtils.isEmpty(userAgent)) {
                    ImsManager.getInstance(mContext, mPhoneId).setImsUserAgent(userAgent);
                }
                //End added by cong.tao.hz for XR7222334 telecomcode on 2018/12/19
                mSimRecordsLoadedRegistrants.notifyRegistrants();
                updateVoiceMail(); //Added by lichun.xiu for XR6623928 tclPlugin on 2018/10/06
                break;
            ...
            case EVENT_SSN:
                ...
                if (isPhoneTypeGsm()) {
                    ...
                    if (mSsnRegistrants.size() == 0) {
                        logd(" Ssn not registrant, need storage Ssn msg");
                        mCachedSsn = ar;
                    }
                    // End added by guoqi.zou for XR7098440&7098464 telecomcode on 2018/11/16
            ...
            case EVENT_SET_VM_NUMBER_DONE:
                ...
                //Begin added by lichun.xiu for XR6623928 tclPlugin on 2018/10/06
                /*
                Some sim card ,such as T-mobile,can set voice mail in sim card successfully,
                if we read preference voice mail first,we also need to update number in preference.
                */
                else if (isPhoneTypeGsm() && ar.exception == null) {
                    boolean isFirstSim = ResUtils.getBoolean(mContext,"def_framework_is_first_read_sim_voice_mail_number",false);
                    if (!isFirstSim) {
                        Rlog.d(LOG_TAG, "EVENT_SET_VM_NUMBER_DONE store voice mail number");
                        storeVoiceMailNumber(mVmNumber);
                    }
                }
                //End added by lichun.xiu for XR6623928 tclPlugin on 2018/10/06
            ...
            case EVENT_GET_RADIO_CAPABILITY:
                ...
                if (ar.exception != null) {
                    ...
                    //Begin added by yangning.hong.hz for XR7076494 telecomcode on 2018/10/31
                    SimLockUtils.setMainPhoneIdIfNeed(rc.getRadioAccessFamily(), rc.getPhoneId());
                    //End added by yangning.hong.hz for XR7076494 telecomcode on 2018/10/31
                }
