...
//Begin added by quantai.zhu for XR6622094 telecomcode on 2018/07/20
import com.tcl.plugin.TclPluginManager;
//End added by quantai.zhu for XR6622094 telecomcode on 2018/07/20
//Begin added by quantai.zhu for XR7041251 telecomcode on 2018/09/30
import android.util.ResUtils;
//End added by quantai.zhu for XR7041251 telecomcode on 2018/09/30
...
public abstract class SMSDispatcher extends Handler {
    ...
    protected void sendMultipartText(String destAddr, String scAddr,
        ...
        //Begin added by quantai.zhu for XR7037496 telecomcode on 2019/09/28
        encoding = TclPluginManager.getTclSmsEncoder().getEncoding(mContext, parts, msgCount);
        //End added by quantai.zhu for XR7037496 telecomcode on 2019/09/28
    ...
    public void sendRawPdu(SmsTracker tracker) {
        //Begin added by lichun.xiu for XR7119137 telecomcode on 2018/11/28
        if(mPhone.getIccCard().getIccFdnEnabled()&&!mPhone.getSubscriberId().startsWith("334020")
                //Begin added by lichun.xiu for XR7448935 telecomcode on 2018/02/20
                &&!ResUtils.getBoolean(mContext,"def_req_disable_sms_fdn",false)) {
                //End added by lichun.xiu for XR7448935 telecomcode on 2018/02/20
            boolean checkSmsc = TclPluginManager.getTclVoicemail(mContext)
                    .checkSMSCforSMS(getSubId(), (String) tracker.getData().get("scAddr"), tracker.mDestAddress);
            Rlog.d(TAG, "checkSmsc:" + checkSmsc);
            if (!checkSmsc) {
                Rlog.d(TAG, "fdn check smsc fail");
                tracker.onFailed(mContext, RESULT_ERROR_FDN_CHECK_FAILURE, 0/*errorCode*/);
                return;
            }
        }
        //End added by lichun.xiu for XR7119137 telecomcode on 2018/11/28
    ...
    protected void handleConfirmShortCode(boolean isPremium, SmsTracker tracker) {
        ...
        //Begin added by quantai.zhu for XR7041251 telecomcode on 2018/09/30
        boolean disablePremiumDialog = ResUtils.getBoolean(mContext,"def_framework_disable_premiumSmsPermission",false);
        Rlog.d(TAG, "handleConfirmShortCode  enableChargeMsg " + disablePremiumDialog);
        if(disablePremiumDialog) {
            sendMessage(obtainMessage(EVENT_SEND_CONFIRMED_SMS, tracker));
            return;
        }
        //Begin added by quantai.zhu for XR7041251 telecomcode on 2018/09/30
