...
public abstract class Phone extends Handler implements PhoneInternalInterface {
        ...
        if (ar.exception == null && ar.result != null) {
            try {
                ...
                if (modes[0] == 0) {
                    ...
                    doAutomatic = false;
                    //Begin added by quan.luo for XR7101642 telecomcode on 2018/11/15
                    //If current state is not in service,select network automatically again
                    Phone mPhone = PhoneFactory.getPhone(mPhoneId);
                    int state = mPhone.getServiceState().getState();
                    if(state!=ServiceState.STATE_IN_SERVICE){
                        doAutomatic = true;
                    }
                    //End added by quan.luo for XR7101642 telecomcode on 2018/11/15
                }
            } catch (Exception e) {
    ...
    public void selectNetworkManually(OperatorInfo network, boolean persistSelection,
        ...
        //Begin modified by quan.luo for XR7139077 telecomcode on 2018/11/21
        //mCi.setNetworkSelectionModeManual(network.getOperatorNumeric(), msg);
        mCi.setNetworkSelectionModeManual(network.getOperatorNumeric() + "+" + network.getRadioTech(), msg);
        //End modified by quan.luo for XR7139077 telecomcode on 2018/11/21
