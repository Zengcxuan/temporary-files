...
//Begin add by chenli.gao.hz for XR7139801 on 2018/11/30
import java.nio.ByteBuffer;
import java.nio.ByteOrder;
import android.os.ServiceManager;
import android.os.RemoteException;
import org.codeaurora.internal.IExtTelephony;
//End add by chenli.gao.hz for XR7139801 on 2018/11/30

// Begin added by guoqi.zou for XR7032611&7032617&7032618&7032646 tclPlugin telecomcode on 2018/09/25
import com.tcl.plugin.TclPluginManager;
// End added by guoqi.zou for XR7032611&7032617&7032618&7032646 tclPlugin telecomcode on 2018/09/25
...
public class GsmCdmaConnection extends Connection {
    ...
    //Begin add by chenli.gao.hz for XR7139801 on 2018/11/30
    private static final int HOOK_TCL_GET_SPEECH_CODEC = 0x80000 + 8001;
    //End add by chenli.gao.hz for XR7139801 on 2018/11/30
    ...
    int disconnectCauseFromCode(int causeCode) {
        ...
        // Begin added by guoqi.zou for XR7032611&7032617&7032618&7032646 tclPlugin telecomcode on 2018/09/25
        int clearCode = TclPluginManager.getTclCallsManager(mOwner.getPhone()
                .getContext()).disconnectCauseFromCode(causeCode, -1);
        if(clearCode != -1){
            return clearCode;
        }
        // End added by guoqi.zou for XR7032611&7032617&7032618&7032646 tclPlugin telecomcode on 2018/09/25
        ...
        switch (causeCode) {
            ...
            case CallFailCause.IMEI_NOT_ACCEPTED:
                //Begin add by zubai.li for XR7051329 telecomcode on 2018/10/19
                if (mIsEmergencyCall) {
                    return TclPluginManager.getTclCallsManager(null).disconnectCauseCustomize(mOwner.getPhone().getServiceState().getState());
                }
                //End add by zubai.li for XR7051329 telecomcode on 2018/10/19
            ...
            default:
                ...
                //Begin add by zubai.li for XR7051329 telecomcode on 2018/10/19
                    ...
                    int returnCause = TclPluginManager.getTclCallsManager(null).disconnectCauseCustomize(serviceState);
                    if (returnCause != DisconnectCause.NORMAL) {
                        return returnCause;
                    }
                }
                //End add by zubai.li for XR7051329 telecomcode on 2018/10/19
    ...
    //Begin add by chenli.gao.hz for XR7139801 on 2018/11/30
    void trySetAudioQuality() {
        int oldAudioQuality = getAudioQuality();
        int speechCodec = getCurrentSpeechCodec();

        log("oldAudioQuality = " + oldAudioQuality +", speechCodec = " + speechCodec);

        if ((speechCodec == BB_TelephonySpeechCodec.AMR_WB) && (oldAudioQuality != AUDIO_QUALITY_HIGH_DEFINITION)) {
            setAudioQuality(AUDIO_QUALITY_HIGH_DEFINITION);
        } else if ((speechCodec != BB_TelephonySpeechCodec.AMR_WB) && (oldAudioQuality == AUDIO_QUALITY_HIGH_DEFINITION)) {
            setAudioQuality(AUDIO_QUALITY_STANDARD);
        }
    }

    private int getCurrentSpeechCodec() {
        int codec = 0;
        try {
            byte[] payload = new byte[1];
            payload[0] = (byte) getGsmCdmaIndex();

            int phoneId = mOwner.getPhone().getPhoneId();
            log("getCurrentSpeechCodec invokeSyncQcRilHookRequest phoneId = " + phoneId);
            byte [] resp = null;
            IExtTelephony iTelEx = IExtTelephony.Stub.asInterface(ServiceManager.getService("extphone"));
            if (iTelEx != null) {
                resp = iTelEx.invokeSyncQcRilHookRequest(HOOK_TCL_GET_SPEECH_CODEC, payload, phoneId);
            }
            log("getCurrentSpeechCodec invokeSyncQcRilHookRequest, resp = " + resp);

            if (resp == null) {
                return 0;
            }

            ByteBuffer byteBuf = ByteBuffer.wrap(resp);
            byteBuf.order(ByteOrder.nativeOrder());
            codec = byteBuf.getInt();
        } catch (CallStateException ex) {
            log("getCurrentSpeechCodec CallStateException");
        } catch (RemoteException e) {
                log("getCurrentSpeechCodec RemoteException = " + e);
        }

        log("getCurrentSpeechCodec--> codec: " + codec);
        return codec;
    }
    //End add by chenli.gao.hz for XR7139801 on 2018/11/30
