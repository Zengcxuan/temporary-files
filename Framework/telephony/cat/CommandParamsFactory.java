...
class CommandParamsFactory extends Handler {
    ...
    void make(BerTlv berTlv) {
        ...
        try {
            ...
            case ACTIVATE:
                ...
                break;
	      //Begin added by donghai.wu for XR6701073 telecomcode  on 2018/09/28
            case LANGUAGE_NOTIFICATION:
                cmdPending = processLanguageNotification(cmdDet, ctlvs);
                break;
	     //End added by donghai.wu for  XR6701073 telecomcode  on 2018/09/28
            default:
    ...
    //Begin added by donghai.wu for XR6701073 telecomcode  on 2018/09/28
    //Add the command LANGUAGE_NOTIFICATION.
    public boolean processLanguageNotification(CommandDetails cmdDet, List<ComprehensionTlv> ctlvs)throws ResultException {
         TextMessage textMsg = new TextMessage(); // change language also need to be handled
         if(cmdDet.commandQualifier == 0x00){
             textMsg = null;
             mCmdParams = new LanguageNotificationParams(cmdDet, textMsg);
             return false;
         }
         ComprehensionTlv ctlv = searchForTag(ComprehensionTlvTag.LANGUAGE, ctlvs);

         if (ctlv != null) {
            textMsg.text = retrieveLanguageNotification(ctlv);
         } else {
    ...
    public static String retrieveLanguageNotification(ComprehensionTlv ctlv) throws ResultException {
        String language = null;
        byte[] rawValue = ctlv.getRawValue();
        int valueIndex = ctlv.getValueIndex();
        try {
            language = GsmAlphabet.gsm7BitPackedToString(rawValue, valueIndex, 2);
            return language;
    ...
    private boolean processGetInput(CommandDetails cmdDet,
        ...
        // Begin added by donghai.wu  for XR6625796  telecomcode  on 2018/09/28
	  ctlv = searchForTag(ComprehensionTlvTag.DURATION, ctlvs);
	  if (ctlv != null) {
	       input.duration = ValueParser.retrieveDuration(ctlv);
	  } 
	  // End added by donghai.wu  for XR6625796  telecomcode  on 2018/09/28
        input.digitOnly = (cmdDet.commandQualifier & 0x01) == 0;
        input.ucs2 = (cmdDet.commandQualifier & 0x02) != 0;
    ...
    private boolean processSelectItem(CommandDetails cmdDet,
        ...
        if (ctlv != null) {
	      ...
	      // Begin added by donghai.wu  for XR6154319  telecomcode  on 2018/09/28
            TclPluginManager.getTclStk(null).setSetupMenuAsTitle(cmdDet.typeOfCommand,menu.title,((RilMessageDecoder)mCaller).getSlotId());
            // End added by donghai.wu  for XR6154319  telecomcode  on 2018/09/28
