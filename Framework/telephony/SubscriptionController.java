...
import android.os.SystemProperties; //Added by zubin.chen.hz for roaming XR6356004 telecomcode on 2018/07/25
...
import com.tcl.plugin.TclPluginManager; //added by shengjiao.liu.hz for XR7171350 telecomcode on 2018/12/03
//Begin added by quantai.zhu for XR7234890 telecomcode on 2018/12/25
import android.app.ActivityManager;
//End added by quantai.zhu for XR7234890 telecomcode on 2018/12/25
...
public class SubscriptionController extends ISub.Stub {
    ...
    //Begin added by zubin.chen.hz for roaming XR6356004 telecomcode on 2018/07/25
    private static boolean isSpnShowFormatChange = false;
    //End added by zubin.chen.hz for roaming XR6356004 telecomcode on 2018/07/25
    ...
    protected void init(Context c) {
        ...
        //Begin added by zubin.chen.hz for roaming XR6356004 telecomcode on 2018/07/25
        isSpnShowFormatChange = SystemProperties.getBoolean("ro.idea.spn.show.format.enable", false);
        logdl("isSpnShowFormatChange=" + isSpnShowFormatChange);
        //End added by zubin.chen.hz for roaming XR6356004 telecomcode on 2018/07/25
    ...
    public int addSubInfoRecord(String iccId, int slotIndex) {
        ...
        try {
            ...
            if (setDisplayName) {
                ...
                //Begin added by shengjiao.liu.hz for XR7171350 telecomcode on 2018/12/03
                nameToSet = TclPluginManager.getTclCustomizeplmnspn(mContext).CustomizeDisplayName(nameToSet, subId);
                //End added by shengjiao.liu.hz for XR7171350 telecomcode on 2018/12/03
                ContentValues value = new ContentValues();
                ...
                //Begin added by shengjiao.liu.hz for XR7171350 telecomcode on 2018/12/03
                String simNumeric = TelephonyManager.getDefault().getSimOperatorNumeric(subId);
                TclPluginManager.getTclCustomizeplmnspn(mContext).setDisplayNameForSameOperator(
                          SubscriptionManager.from(mContext), simNumeric, slotIndex);
                //End added by shengjiao.liu.hz for XR7171350 telecomcode on 2018/12/03
    ...
    public boolean setPlmnSpn(int slotIndex, boolean showPlmn, String plmn, boolean showSpn,
        ...
        synchronized (mLock) {
                ...
                if (showSpn) {
                    ...
                    if(!Objects.equals(spn, plmn)) {
                        ...
                        if (isSpnShowFormatChange) {
                            separator = " | ";
                            if (DBG) logd("[setPlmnSpn] separator=" + separator);
                        }
                        //End added by zubin.chen.hz for roaming XR6356004 telecomcode on 2018/07/25
            ...
            //Begin added by quan.luo for XR6623823 on 2018/10/05
            //When the def_SPN_PLMN_display_rule is set to 0x05 and the UE is out of the service,
            //the showSpn and showplmn would both be false and carrierText = "".
            if (!showSpn && (!showPlmn)){
                //Begin added by quan.luo for XR7116972 on 2018/11/15
                //So we so should set the plmn to carrierText
                carrierText = plmn;
                //End added by quan.luo for XR7116972 on 2018/11/15
            }
            //End added by quan.luo for XR6623823 on 2018/10/05
    ...
    public int getDefaultSmsSubId() {
        ...
        //Begin added by quantai.zhu for XR7234890 telecomcode on 2018/12/25
        //if subID<0 when not any sim insert,Google Messager can not delete sms,return 0 to fix it.
        if (subId < 0 && !hasSimInsert() && "com.google.android.apps.messaging".equals(getCallingPackageName())) {
            logd("getDefaultSmsSubId() fix return value for Google Messager ,return 0");
            return 0;
        }
        //End added by quantai.zhu for XR7234890 telecomcode on 2018/12/25
        return subId;
    ...
    private String getCallingPackageName(){
        ...
        for (ActivityManager.RunningAppProcessInfo info : rap){
            if (info.pid == callingPid) return info.processName;
        }
        return null;
    }
    private boolean hasSimInsert(){
        ...
        for (int i=0;i<phoneCount;i++) {
            if(mTelephonyManager.hasIccCard(i)) return true;
        }
        return false;
    }
    //End added by quantai.zhu for XR7234890 telecomcode on 2018/12/25
