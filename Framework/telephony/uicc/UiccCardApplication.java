...
import com.tcl.plugin.TclPluginManager; //Added by cong.tao.hz for XR7046534 tclPlugin telecomcode on 2018/10/11
...
public class UiccCardApplication {
    ...
    private void onChangeFdnDone(AsyncResult ar) {
        synchronized (mLock) {
            ...
            if (ar.exception == null) {
                if (DBG) log("notifyFdnChangedRegistrants");
                TclPluginManager.getTclFdn(getPhoneId()).onChangeFdnDone();
            }
        ...
        public void handleMessage(Message msg){
            ...
            switch (msg.what) {
                case EVENT_PIN1_PUK1_DONE:
                     //Begin added yangning.hong.hz for XR7225693 telecomcode on 2018/12/19
                     queryPin1State();
                     //End added yangning.hong.hz for XR7225693 telecomcode on 2018/12/19
    ...
    private void notifyNetworkLockedRegistrantsIfNeeded(Registrant r) {
        ...
        if (mAppState == AppState.APPSTATE_SUBSCRIPTION_PERSO &&
                isPersoLocked()) {
            //Begin added by zubin.chen.hz for simlock SIMLOCK_UI on telecomcode on 2018/10/12
            //AsyncResult ar = new AsyncResult(null, mPersoSubState.ordinal(), null);
            int[] result = {mUiccCard==null?-1:mUiccCard.getPhoneId(), mPersoSubState.ordinal()};
            AsyncResult ar = new AsyncResult(null, result, null);
            //End added by zubin.chen.hz for simlock SIMLOCK_UI on telecomcode on 2018/10/12
            if (r == null) {
