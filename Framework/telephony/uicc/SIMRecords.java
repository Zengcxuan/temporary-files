...
//Begin added by yangning.hong.hz for XR7035711 tclPlugin telecomcode on 2018/09/26
import com.tcl.plugin.TclPluginManager;
import com.tcl.plugin.ITclSIMRecordsCallback;
//End added by yangning.hong.hz for XR7035711 tclPlugin telecomcode on 2018/09/26

//Begin added by yangning.hong.hz for XR7035699 telecomcode on 2018/10/22
import android.os.SystemProperties;
import com.tcl.plugin.SimLockUtils;
//End added by yangning.hong.hz for XR7035699 telecomcode on 2018/10/22
...
public class SIMRecords extends IccRecords {
    ...
    // Begin added by donghai.wu  for XR6154319  telecomcode  on 2018/09/28
    private int mSlotId =0;
    // End added by donghai.wu  for XR6154319  telecomcode  on 2018/09/28
    ...
    // Begin added by donghai.wu  for XR6154319  telecomcode  on 2018/09/28
    private static final int EVENT_QUERY_MENU_TITLE_DONE = 60 + SIM_RECORD_EVENT_BASE;
    // End added by donghai.wu  for XR6154319  telecomcode  on 2018/09/28
    ...
    protected void resetRecords() {
        ...
        //Begin added by yangning.hong.hz for XR7035699 telecomcode on 2018/10/22
        SystemProperties.set(SimLockUtils.PROPERTY_SIM_IMSI[mParentApp.getPhoneId()], "");
        SystemProperties.set(SimLockUtils.PROPERTY_SIM_GID1[mParentApp.getPhoneId()], "");
        //End added by yangning.hong.hz for XR7035699 telecomcode on 2018/10/22
    ...
    //Begin modified by yadong.yang for XR7040266 on 20180928
	...
	for(i = bs.length*2; i<32; i++) {
            builder.append('F');
	}
        return builder.toString().trim();
    }
    //End modified by yadong.yang for XR7040266 on 20180928
    ...
    public void handleMessage(Message msg) {
	  ...
	  // Begin added by donghai.wu  for XR6154319  telecomcode  on 2018/09/28
        String mMenuTitleFromEf = null;
        // End added by donghai.wu  for XR6154319  telecomcode  on 2018/09/2
        ...
        try {
            switch (msg.what) {
                ...
                case EVENT_GET_IMSI_DONE:
                    ...
                    //Begin added by yangning.hong.hz for XR7035699 telecomcode on 2018/10/22
                    SystemProperties.set(SimLockUtils.PROPERTY_SIM_IMSI[mParentApp.getPhoneId()], imsi);
                    //End added by yangning.hong.hz for XR7035699 telecomcode on 2018/10/22
                    ...
                    if (mMncLength != UNKNOWN && mMncLength != UNINITIALIZED
                        ...
                        //Begin modified by yangning.hong.hz for XR7229150 telecomcode on 2018/12/18
                        SystemProperties.set(SimLockUtils.PROPERTY_RIL_SIM_UICC_MCCMNC[mParentApp.getPhoneId()], imsi.substring(0, 3 + mMncLength));
                        //End modified by yangning.hong.hz for XR7229150 telecomcode on 2018/12/18
                    }
                ...
                case EVENT_GET_SPN_DONE:
                    ...
                    SystemProperties.set(SimLockUtils.PROPERTY_SIM_SPN[mParentApp.getPhoneId()], StrToHex(spn));
                    //End modified by yadong.yang for XR7040266 on 20180928
                    //End modified by yangning.hong.hz for XR7229150 telecomcode on 2018/12/18
                    break;
                ...
                case EVENT_GET_PNN_DONE:
                    ...
                    for (; tlv.isValidObject(); tlv.nextObject()) {
                        if (tlv.getTag() == TAG_FULL_NETWORK_NAME) {
                            ...
                            //Begin added by quan.luo for XR7046680 telecomcode on 2018/10/13
                            TclPluginManager.getTclCustomizeplmnspn(mContext).SetDisplayNameUsingPNN(
                                    mPnnHomeName, mImsi, mParentApp.getPhoneId());
                            //End added by quan.luo for XR7046680 telecomcode on 2018/10/13
                            break;
                        }
                    }
                    break;
                ...
                case EVENT_GET_GID1_DONE:
                    ...
                    //Begin added by yangning.hong.hz for XR7035699 telecomcode on 2018/10/22
                    SystemProperties.set(SimLockUtils.PROPERTY_SIM_GID1[mParentApp.getPhoneId()], mGid1);
                    //End added by yangning.hong.hz for XR7035699 telecomcode on 2018/10/22
                    log("GID1: " + mGid1);
		   ...
		   // Begin added by donghai.wu  for XR6154319  telecomcode  on 2018/09/28
                ...
                */
                case EVENT_QUERY_MENU_TITLE_DONE:
                    loge("[sume receive response message");
                    isRecordLoadResponse = true;
                    ar = (AsyncResult) msg.obj;
                    if (ar != null && ar.exception == null) {
                        ...
                        if (data != null && data.length >= 2) {
                            int tag = data[0] & 0xff;
                            int len = data[1] & 0xff;
                            loge("[sume tag = " + tag + ", len = " + len);
				   mMenuTitleFromEf = IccUtils.adnStringFieldToString(data, 2, len);
                            mMenuTitleFromEf = IccUtils.adnStringFieldToString(data, 2, len);
                            loge("[sume menu title is " + mMenuTitleFromEf);
			        TclPluginManager.getTclStk(mContext).setEfsumeAsTitle(mMenuTitleFromEf,mSlotId);
                       }
                    } else {
                        loge("[sume null AsyncResult or exception.");
    ...
    /**
     * Added by quan.luo for getting pnn in record # 1 for XR7043116 telcecomcode on 2018/10/08
     * {@hide}
     * @return
     */
    public String getFirstFullNameInEfPnn() {
        Rlog.d("Simrecord","mPnnHomeName: " + mPnnHomeName);
        return mPnnHomeName;
    }
    ...
    protected void onAllRecordsLoaded() {
        ...
        //Begin modified by yangning.hong.hz for XR7035711 tclPlugin telecomcode on 2018/09/26
        ...
        TclPluginManager.getTclCustomLanguage(mContext)
                .setLanguageAndLocaleFromSIM(mEfLi, mEfPl, this, mRecordsCallback);
        //End modified by yangning.hong.hz for XR7035711 tclPlugin telecomcode on 2018/09/26
        ...
        if (!TextUtils.isEmpty(operator)) {
            ...
            // Begin added by donghai.wu  for XR6154319  telecomcode  on 2018/09/28
	     TclPluginManager.getTclStk(mContext).sendResetStk(mContext);
           // End added by donghai.wu  for XR6154319  telecomcode  on 2018/09/28
        } else {
    ...
    private void setDisplayName() {
        ...
        if (subInfo != null && subInfo.getNameSource() !=
            ...
            if (!TextUtils.isEmpty(newCarrierName) && !newCarrierName.equals(oldSubName)) {
                ...
                //Begin added by quantai.zhu for XR7115050 telecomcode on 2018/11/22
                newCarrierName = TclPluginManager.getTclCustomizeplmnspn(mContext).CustomizeDisplayName(newCarrierName, subId[0]);
                //End added by quantai.zhu for XR7115050 telecomcode on 2018/11/22

                SubscriptionController.getInstance().setDisplayName(newCarrierName, subId[0]);

                //Begin added by quantai.zhu for XR7115050 telecomcode on 2018/11/22
                TclPluginManager.getTclCustomizeplmnspn(mContext).setDisplayNameForSameOperator(subManager,
                    mTelephonyManager.getSimOperatorNumeric(subId[0]), mParentApp.getPhoneId());
                //End added by quantai.zhu for XR7115050 telecomcode on 2018/11/22
            }
        } else {
    ...
    private void setSpnFromConfig(String carrier) {
        //Begin modified by quan.luo for XR5953869 tclplugin on 2018/02/09
        if (mSpnOverride.containsCarrier(carrier) && TextUtils.isEmpty(getServiceProviderName())) {
            //old:
            ...
            String spn = TclPluginManager.getTclCustomizeplmnspn(null).CustomizeSpn(
                    mSpnOverride.getSpn(carrier), mPnnHomeName, carrier, mImsi);
            mTelephonyManager.setSimOperatorNameForPhone(mParentApp.getPhoneId(), spn);
        }
        //End modified by quan.luo for XR5953869 tclplugin on 2018/02/09
    }
    ...
    protected void fetchSimRecords() {
	  ...
	  // Begin added by donghai.wu  for XR6154319  telecomcode  on 2018/09/28
	  mRecordsToLoad =TclPluginManager.getTclStk(mContext).loadEfsumeForPerso(mFh,this,mRecordsToLoad);
	  // End added by donghai.wu  for XR6154319  telecomcode  on 2018/09/28
    ...
    //Begin added by yangning.hong.hz for XR7035711 tclPlugin telecomcode on 2018/09/26
    ITclSIMRecordsCallback mRecordsCallback = new ITclSIMRecordsCallback() {
          @Override
          public void setSimLanguageCallback(byte[] efLi, byte[] efPl) {
              setSimLanguage(efLi, efPl);
          }
    };
    //End added by yangning.hong.hz for XR7035711 tclPlugin telecomcode on 2018/09/26
}
