...
//Begin added by lichun.xiu for XR7341908 telecomcode on 2019/01/22
import android.text.TextUtils;
//End added by lichun.xiu for XR7341908 telecomcode on 2019/01/22
...
public class VisualVoicemailSmsParser {
    ...
    private static Bundle parseSmsBody(String message) {
        ...
        for (String entry : entries) {
            ...
            //Begin added by lichun.xiu for XR7341908 telecomcode on 2019/01/22
            if(!TextUtils.isEmpty(value)&&value.length()==12&&value.startsWith("1+")){
                android.util.Log.d("VisualVoicemailSmsParser","vvm1 number:"+value);
                value = value.replace("1+","+1");
                android.util.Log.d("VisualVoicemailSmsParser","vvm2 number:"+value);
            }else if(!TextUtils.isEmpty(value)&&value.length()==28
                    &&value.contains("/")&&value.contains("-")){
                int index = value.indexOf("-");
                int lastindex = value.lastIndexOf("-");
                android.util.Log.d("VisualVoicemailSmsParser","vvm1 time:"+value);
                if(lastindex!=index){
                    String valuelast = value.substring(lastindex);
                    String valuefirst = value.substring(0,index);
                    value = valuefirst+valuelast;
                }
                android.util.Log.d("VisualVoicemailSmsParser","vvm2 time:"+value);
            }
            //End added by lichun.xiu for XR7341908 telecomcode on 2019/01/22
