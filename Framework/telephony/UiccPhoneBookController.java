...
public class UiccPhoneBookController extends IIccPhoneBook.Stub {
    ...
    @Override
    public int getInsertRecordIndex() throws android.os.RemoteException {
        return getInsertRecordIndexForSubscriber(getDefaultSubscription());
    }

    @Override
    public int getInsertRecordIndexForSubscriber(int subId)
           throws android.os.RemoteException {
        IccPhoneBookInterfaceManager iccPbkIntMgr =
                             getIccPhoneBookInterfaceManager(subId);
        if (iccPbkIntMgr != null) {
            return iccPbkIntMgr.getInsertRecordIndex();
        } else {
            Rlog.e(TAG,"getInsertRecordIndex iccPbkIntMgr is" +
                      " null for Subscription:"+subId);
            return -1;
        }
    }
    ...
    public boolean isAdnLoadDone() throws android.os.RemoteException {
        return isAdnLoadDoneForSubscriber(getDefaultSubscription());
    }

    @Override
    public boolean isAdnLoadDoneForSubscriber(int subId)
           throws android.os.RemoteException {
        IccPhoneBookInterfaceManager iccPbkIntMgr =
                             getIccPhoneBookInterfaceManager(subId);
        if (iccPbkIntMgr != null) {
            ...
            Rlog.e(TAG,"isAdnLoadDone iccPbkIntMgr is" + " null for Subscription:"+subId);
            return false;
        }
    }
