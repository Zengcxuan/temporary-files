...
import com.tcl.plugin.ITclCustomizeplmnspn;
import com.tcl.plugin.TclPluginManager; //Added by zubin.chen.hz for roaming tclPlugin telecomcode on 2018/08/08
...
public class ServiceStateTracker extends Handler {
    ...
    //Begin added by quan.luo for XR7042684 tclPlugin telecomcode on 2018/10/05
    private ITclCustomizeplmnspn mTclCustomizeplmnspn = null;
    //End added by quan.luo for XR7042684 tclPlugin telecomcode on 2018/10/05
    ...
    */
    ...
    private class SstSubscriptionsChangedListener extends OnSubscriptionsChangedListener {
        ...
        public void onSubscriptionsChanged() {
            ...
            if (mPreviousSubId.getAndSet(subId) != subId) {
                ...
                //Begin deleted by lichun.xiu for XR7407358 telecomcode on 2019/01/23
                log("Deleted ServiceStateTracker mPhone.updateVoiceMail");
    ...
    public ServiceStateTracker(GsmCdmaPhone phone, CommandsInterface ci) {
        ...
        //Begin added by quan.luo for XR7042684 tclPlugin telecomcode on 2018/10/05
        mTclCustomizeplmnspn = TclPluginManager.getTclCustomizeplmnspn(mPhone.getContext());
        //End added by quan.luo for XR7042684 tclPlugin telecomcode on 2018/10/05
    }
    ...
    protected void handlePollStateResultMessage(int what, AsyncResult ar) {
        ...
        switch (what) {
            ...
            case EVENT_POLL_STATE_GPRS: {
                ...
                //Begin added by zubin.chen.hz for roaming tclPlugin telecomcode on 2018/07/24
                if(TclPluginManager.getTclRoaming(mPhone.getContext()).isIgnorePsSearchState(regState)) {
                    return;
                }
                //End added by zubin.chen.hz for roaming tclPlugin telecomcode on 2017/07/24
    ...
    protected void updateRoamingState() {
        if (mPhone.isPhoneTypeGsm()) {
            ...
            //Begin added by zubin.chen.hz for roaming tclPlugin telecomcode on 2018/07/24
            roaming = TclPluginManager.getTclRoaming(mPhone.getContext()).isRoaming(roaming, mSubId, mNewSS.getOperatorNumeric(), getSIMOperatorNumeric());
            //End added by zubin.chen.hz for roaming tclPlugin telecomcode on 2018/07/24
    ...
    protected void updateSpnDisplay() {
        ...
        if (mPhone.isPhoneTypeGsm()) {
            ...
            if (combinedRegState == ServiceState.STATE_OUT_OF_SERVICE
                ...
                realPlmn = plmn; //Added by quan.luo for XR7042684 tclPlugin telecomcode on 2018/10/05
            } else {
            ...
            if (!TextUtils.isEmpty(spn) && !TextUtils.isEmpty(wfcVoiceSpnFormat) &&
                ...
                plmn = String.format(wfcVoiceSpnFormat, originalPlmn);
                    ...
                    //Begin modified by quan.luo for XR6346131 telecomcode on 2018/08/06
                    //|| (showPlmn && TextUtils.equals(spn, plmn))) {
                    || (showPlmn && (spn != null)&& spn.equalsIgnoreCase(plmn))) {
                //End modified by quan.luo for XR6346131 telecomcode on 2018/08/06
            ...
            //Begin added by quan.luo for XR7042684 tclPlugin telecomcode on 2018/10/05
            mTclCustomizeplmnspn.CustomizePlmnSpn(mPhone.getPhoneId(), plmn, spn, realPlmn, showPlmn, showSpn);
            plmn = mTclCustomizeplmnspn.getCustomizedPlmn();
            spn  = mTclCustomizeplmnspn.getCustomizedSpn();
            realPlmn = mTclCustomizeplmnspn.getCustomizedRealPlmn();
            showPlmn = mTclCustomizeplmnspn.isShowPlmn();
            showSpn  = mTclCustomizeplmnspn.isShowSpn();
            String vivoPlmn = mTclCustomizeplmnspn.getVivoPlmn();
            log("plmn = " + plmn + " , spn = " + spn + " , realPlmn = " + realPlmn +
                    " , showPlmn = " + showPlmn + " , showSpn = " + showSpn);
            //End added by quan.luo for XR7042684 tclPlugin telecomcode on 2018/10/05
            ...
            if (mSubId != subId ||
                ...
                //Begin added by quan.luo for XR7042684 tclPlugin telecomcode on 2018/10/05
                intent.putExtra("vivoPlmnSpn", vivoPlmn);
                intent.putExtra("OrangeShowPlmn", mTclCustomizeplmnspn.isOrangeShowPlmn());
                //End added by quan.luo for XR7042684 tclPlugin telecomcode on 2018/10/05
    ...
    private void pollStateDone() {
        ...
        if (hasChanged) {
            ...
            if (isInvalidOperatorNumeric(operatorNumeric)) {
                ...
                //modify by yeqing.lv for defect P10024957 on 2018-10-27 begin
                // if (!mNitzUpdatedTime && !mcc.equals("000") && !TextUtils.isEmpty(iso)
                //       && getAutoTimeZone()) {
                TimeZone zone = null;
                if (!mNitzUpdatedTime && !mcc.equals("000") && !TextUtils.isEmpty(iso) ) {
                //modify by yeqing.lv for defect P10024957 on 2018-10-27 end
                    ...
                    if ((uniqueZoneIds.size() == 1) || testOneUniqueOffsetPath) {
                      ...
                      //  setAndBroadcastNetworkSetTimeZone(zoneId);
                        ...
                        String zoneId = uniqueZoneIds.get(0);
                        if (DBG) {
                        log("mNeedFixZoneAfterNitz=" + mNeedFixZoneAfterNitz+
                                " mZoneOffset= " + mZoneOffset +
                                " mZoneDst="+mZoneDst + " mZoneTime=" + mZoneTime +
                                " iso=" + iso);
                        }
                        if ((mNeedFixZoneAfterNitz) || ((mZoneOffset == 0) && (mZoneDst == false))) {
                            zone = getTimeZonesWithCapitalCity(iso);
                        }
                        if (zone != null) {
                            if (DBG) {
                                log("pollStateDone:Find the capital city with zone.getID=" + zone.getID() +
                            ...
                            log("Cann't find the capital city, do nothing");
                        }
                    //modify by yeqing.lv for defect P10024957 on 2018-10-27 end
                    } else {
                        if (DBG) {
        ...
        TclPluginManager.getTclRoaming(mPhone.getContext())
                .onRoamingStateChanged(hasVoiceRoamingOn, hasVoiceRoamingOff, hasDataRoamingOn, hasDataRoamingOff, mPhone.getPhoneId());
    ...
    //add by yeqing.lv for defect P10024957 on 2018-10-27 begin
    //UE can not update Timezone automaticlly because operator does not sent NITZ message
    //and the country has more than one timezones. So set a default a timezone
    //on this occasion.
    //Get the default timezone of a country.
    private TimeZone getTimeZonesWithCapitalCity(String iso){
        ...
        for(int i = 0; i < mTimeZoneIdOfCapitalCity.length; i++){
             if(iso.equals(mTimeZoneIdOfCapitalCity[i][0])){
                 ...
                 log("use TimeZone of Capital City:"+mTimeZoneIdOfCapitalCity[i][1]);
                 break;
             }
         }
        return tz;
    }
    //add by yeqing.lv for defect P10024957 on 2018-10-27 end
    ...
    //and the country has more than one timezones. So set a default a timezone
    //on this occasion.
    //Default Timezone for countries which have more than one timezones.
